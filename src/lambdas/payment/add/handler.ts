...
export const lambdaHandler = lambda_wrapper_json([add_payment],
    async (lambda_event, db, config, current_user) => {
        ...
        await postToDiscord(config, `${current_user.userName} added a${lambda_event.body.type === 'adjustment' ? 'n': ''} ${lambda_event.body.type} to booking ${booking!.district} of £${lambda_event.body.amount} (${lambda_event.body.note})`)

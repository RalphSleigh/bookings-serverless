...
export const lambdaHandler = lambda_wrapper_json([book_event, book_into_organisation],
    async (lambda_event, db, config, current_user) => {
        ...
        await postToDiscord(config, `${booking.userName} (${booking.district}) created a booking for event ${booking.event!.name}, they have booked ${booking.participants!.length} people`)

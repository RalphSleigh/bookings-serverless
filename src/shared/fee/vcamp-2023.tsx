                ...
                <FormGroup row>
                    ...
                    <Col sm={8}>
                        <InputGroup>
                            ...
                            <Input type="number" className="form-control" value={b.wholeu18}
                                   onChange={this.updateAmount(b.id)('wholeu18')}/>
                        </InputGroup>
                    </Col>
                </FormGroup>
                ...
                <FormGroup row>
                    ...
                    <Col sm={4}>
                        <InputGroup>
                            ...
                            <Input type="number" className="form-control" placeholder="35" value={b.au18}
                                   onChange={this.updateAmount(b.id)('au18')}/>
                        </InputGroup>
                    </Col>
                    <Label sm={2}>B:</Label>
                    <Col sm={4}>
                        <InputGroup>
                            ...
                            <Input type="number" className="form-control" placeholder="35" value={b.bu18}
                                   onChange={this.updateAmount(b.id)('bu18')}/>
                        </InputGroup>
                    </Col>
                </FormGroup>
                <Row><Col><h3>O18</h3></Col></Row>
                <FormGroup row>
                    <Label sm={4}>Whole Event:</Label>
                    <Col sm={8}>
                        <InputGroup>
                            ...
                            <Input type="number" className="form-control" value={b.wholeo18}
                                   onChange={this.updateAmount(b.id)('wholeo18')}/>
                        </InputGroup>
                    </Col>
                </FormGroup>
                <p>Partial attendance fee = A + B * (nights attending)</p>
                <FormGroup row>
                    ...
                    <Col sm={4}>
                        <InputGroup>
                            ...
                            <Input type="number" className="form-control" placeholder="35" value={b.ao18}
                                   onChange={this.updateAmount(b.id)('ao18')}/>
                        </InputGroup>
                    </Col>
                    <Label sm={2}>B:</Label>
                    <Col sm={4}>
                        <InputGroup>
                            ...
                            <Input type="number" className="form-control" placeholder="35" value={b.bo18}
                                   onChange={this.updateAmount(b.id)('bo18')}/>
                        </InputGroup>
                    </Col>
                </FormGroup>
            </React.Fragment>;
...
const owedFreeEvent = (event, participants, booking, payments) => {
    ...
    const u18participants = filteredParticipants.filter(p => Moment(event.startDate).diff(Moment(p.age), 'years') < 18)
    const o18participants = filteredParticipants.filter(p => Moment(event.startDate).diff(Moment(p.age), 'years') >= 18)
    ...
    const rawCostsu18 = u18participants.map(p => sortedBuckets.reduce((a, c) => {
        if (Moment(p.createdAt).isBefore(Moment(c.date).add(1,"days"))) return {
            ...
            date: c.date,
            fee:  p.days === wholeMask ? parseInt(c.wholeu18) : parseInt(c.au18) + (parseInt(c.bu18) * (popcount(p.days) - 1))
        };
        else return a;
    }, {}));

    const combinedCostsu18 = rawCostsu18.reduce((a, c) => {
    ...
    const linesu18 = reduce(combinedCostsu18, (a, c, i) => [...a, ...reduce(c, (a1, c1, i1) => [...a1, {
        //@ts-ignore
        line:  `${c1.count} ${c1.count > 1 ? 'people' : 'person'} under 18 booked for ${c1.type === 'whole' ? 'whole event' : `${i1 - 1} nights`} before ${Moment(i).format('MMMM Do YYYY')} at £${c1.fee} (discounted £${c1.fee/2})`,
    ...
    const rawCostso18 = o18participants.map(p => sortedBuckets.reduce((a, c) => {
        if (Moment(p.createdAt).isBefore(Moment(c.date).add(1,"days"))) return {
            ...
            days: popcount(p.days),
            date: c.date,
            fee:  p.days === wholeMask ? parseInt(c.wholeo18) : parseInt(c.ao18) + (parseInt(c.bo18) * (popcount(p.days) - 1))
        };
        else return a;
    }, {}));

    const combinedCostso18 = rawCostso18.reduce((a, c) => {
    ...
    //@ts-ignore
    const lineso18 = reduce(combinedCostso18, (a, c, i) => [...a, ...reduce(c, (a1, c1, i1) => [...a1, {
        //@ts-ignore
        line:  `${c1.count} ${c1.count > 1 ? 'people' : 'person'} over 18 booked for ${c1.type === 'whole' ? 'whole event' : `${i1 - 1} nights`} before ${Moment(i).format('MMMM Do YYYY')} at £${c1.fee}`,
        total: c1.count * c1.fee
    }], [])], []);
